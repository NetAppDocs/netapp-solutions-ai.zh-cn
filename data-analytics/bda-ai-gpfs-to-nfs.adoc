...
summary: 本节提供使用NetApp XCP 配置 GPFS 和将数据移动到 NFS 所需的详细步骤。 
---
= GPFS 转 NFS-详细步骤
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
本节提供使用NetApp XCP 配置 GPFS 和将数据移动到 NFS 所需的详细步骤。
...
....
. 指定 GPFS 使用的远程 shell 二进制文件；使用 `-r argument`。
+
....
...
[root@mastr-51 installer]#
....
. 指定 GPFS 使用的远程文件复制二进制文件；使用 `-rc argument`。
+
....
...
[root@mastr-51 installer]#
....
. 指定所有 GPFS 节点上要设置的端口范围；使用 `-e argument`。
+
....
